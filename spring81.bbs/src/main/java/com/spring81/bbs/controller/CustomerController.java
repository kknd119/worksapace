...
public class CustomerController {
    ...
    public String login( Model model,@RequestParam(value="url",defaultValue="") String url,HttpServletRequest request) {
        logger.info("/customer/login:get");
    ...
    public String loginp( Model model,@RequestParam String url
        ...
        logger.info("/customer/login:post");
        //로그인 후 이동할 페이지를 변수로  넘긴다.
	...
	public String logout(Model model,HttpSession session) {
	    logger.info("/customer/logout :get");
	    //세션삭제
	session.removeAttribute(WebConstants.SESSION_NAME );
	
	return "redirect:/";
	}
	@RequestMapping(value = "/customer/registerterm", method = RequestMethod.GET)
    public String registerterm(Model model,HttpSession session) {
        logger.info("/customer/registerterm:get");
        //세션삭제
    session.removeAttribute(WebConstants.SESSION_NAME );
    
    return "customer/registerterm";
    }
	@RequestMapping(value = "/customer/registerterm", method = RequestMethod.POST)
    public String registertermp(Model model,HttpSession session) {
        logger.info("/customer/registerterm:post");
        //세션삭제
    session.removeAttribute(WebConstants.SESSION_NAME );
    
    return "shop/registerc";
    }
	@RequestMapping(value = "/customer/registerc", method = RequestMethod.GET)
    public String resister(Model model,HttpSession session) {
        logger.info("/customer/resisterc:get");
        //세션삭제
    session.removeAttribute(WebConstants.SESSION_NAME );
    
    return "customer/registerc";
    }
	//////////////////////////////////////////////
	
	
	@RequestMapping(value = "/customer/unregister", method = RequestMethod.GET)
    public String unregister(Model model ) {
        logger.info("/user/resister:get");
      
    return "customer/unregisterc";
    }
	@RequestMapping(value = "/customer/unregister", method = RequestMethod.POST)
    public String unregisterp(Model model
            ,@RequestParam String email
            ,@RequestParam String passwd
            ,HttpSession session) {
        logger.info("/customer/resister:post");
      ...
      //탈퇴처리 성공하면 세션삭제
        if(result==1) {
            session.removeAttribute(WebConstants.SESSION_NAME);
            return"customer/unregister_postc";
        }
        else {
            return"customer/unregisterc";
        }
    }
	@RequestMapping(value = "/customer/usermodify", method = RequestMethod.GET)//로그인이 되어 있다는 가정하에
    public String usermodify(Model model,HttpSession session) {
        logger.info("/customer/usermodify:get");
        ////로그인이 되어 있다는 가정하에 세션정보 받음
    ModelUser user = (ModelUser)session.getAttribute(WebConstants.SESSION_NAME );
    //DB에서 회원 정보 조회
    //user=svruser.selectUserOne(user);
    if(user == null) {
        return "redirect:/customer/loginc";
    }
    model.addAttribute("user",user);
    return "customer/usermodifyc";
    }
	@RequestMapping(value = "/customer/usermodify", method = RequestMethod.POST)
    public String usermodifyp(Model model,@ModelAttribute ModelUser setValue ,HttpSession session,RedirectAttributes rttr ) {
        logger.info("/customer/usermodify:post");
    ////로그인이 되어 있다는 가정하에 세션정보 받음
        ModelUser user =(ModelUser) session.getAttribute(WebConstants.SESSION_NAME );
        ...
        if(result>0) {
            ...
            rttr.addFlashAttribute("customer",setValue);
          
            rttr.addFlashAttribute("msg",WebConstants.UPDATE_FAIL);
            return "redirect:/customer/customermodify";        
        }
    
    }
	@RequestMapping(value = "/customer/changepassword", method = RequestMethod.GET)
    public String changepassword(Model model,HttpSession session) {
        logger.info("/customer/changepassword:get");
        //세션삭제
    ModelUser customer=(ModelUser) session.getAttribute(WebConstants.SESSION_NAME );
    ...
    public String changepasswordp(Model model
        ...
        int result = svruser.updatePasswd(customer.getUserid(),currentPasswd,newPasswd);
        if(result==1)
            return"customer/changepassword_post";
        else {
	...
	//프로필 사진까지 같이 저장.
	@RequestMapping(value = "/customer/registerc", method = RequestMethod.POST)
    public String resisterp(Model model,@ModelAttribute ModelCustomer customer) {
        logger.info("/customer/resisterc:post");
        
        Integer userno = null;
        try {
            customer.setFileName(customer.getImage().getOriginalFilename());
            customer.setFileSize((Long) customer.getImage().getSize());
            customer.setContentType(customer.getImage().getContentType()); // 확장자
            customer.setImageBytes(customer.getImage().getBytes());
            customer.setImageBase64(Base64.getEncoder()
                    .encodeToString(customer.getImage().getBytes()));
            userno = svruser.insertUserc(customer);
        } catch (Exception e) {
            e.printStackTrace();
        }
        ModelCustomer result =svruser.selectUserOnec(customer);
        model.addAttribute("customer", result);
         
    return "customer/register_postc";
    }
	//프로필 확인
    @RequestMapping(value = "/customer/profile/{userno}", method = RequestMethod.GET)
    public String profile(Model model,@PathVariable Integer userno) {
        //
        logger.info("/customer/profile:get~");
