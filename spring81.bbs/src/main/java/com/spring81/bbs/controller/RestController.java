...
public class RestController {
	...
	@RequestMapping(value = "/checkuseridc", method = {RequestMethod.GET,RequestMethod.POST} )
    @ResponseBody
    public int checkuseridc(String userid) {
        //logger.info("/rest/checkuserid");
                  
        return usersvr.checkuseridc(userid);
    }
	...
	//장바구니에 담는 역할
	@RequestMapping(value = "/insertgetgood", method = {RequestMethod.GET,RequestMethod.POST} )
    @ResponseBody   
    public int insertGetGood ( Model model ,@RequestBody ModelArticle article) {
        logger.info("/rest/insertgetgood");
    int result = boardsvr.insertArticle(  article);
    return  result;
    }
